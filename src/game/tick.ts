...
export function createTickCtx(s: GameState, dt: number): TickCtx {
  const derived = derivedFromPads(s.padsDone);
  const c: TickCtx = {
    dt,
    s,
    ...
    padsDone: s.padsDone,
    padFills: s.padFills,
    // D-015: tables/stations/hasWaiter/hasDishwasher padsDone'dan TÜRETİLİR (kare anlık görüntüsü;
    // kare içinde ayrıca mutasyona uğramaz — pad açılınca yalnız padsDone büyür, gerisi türetilir).
    derived,
    ...
    obstacles: tableSolids(derived.tables),
    // Personel (garson/bulaşıkçı) BFS ızgarası — masa+zone sayısına göre cache'li.
    navGrid: getNavGrid(derived.tables, derived.zonesOpen),
    upgradeFills: s.upgradeFills.slice(),
    activeZone: null,
    stationLevels: s.stationLevels.slice(), // zone başına ocak seviyesi (bu karede yükselebilir)
    tableLevels: s.tableLevels.slice(), // masa-başı seviyeler (kopya; bu karede yükseltilebilir)
    readyCupsByZone: s.readyCupsByZone.slice(),
    brewProgressByZone: s.brewProgressByZone.slice(),
    tray: s.tray,
    trayFood: s.trayFood,
    ...
    // Kalıcı eylem sayaçları (bu karede artabilir). waiterServedByZone dizisi de klonlanır (v21).
    stats: { ...s.stats, waiterServedByZone: s.stats.waiterServedByZone.slice() },
    questIndex: s.questIndex,
    questBase: s.questBase,
    camFocus: s.camFocus,
    // Kamera odak tetikleri aynı karede üst üste binebilir (reveal + görev geçişi + zone açılışı) —
    // 2026-06-11 fix: kare-içi ÖNCELİK (reveal 1 < görev 2 < zone 3); düşük öncelik yükseği EZEMEZ.
    camPrio: 0,
    ...
    padGate: { padsDone: s.padsDone, tables: derived.tables, stationLevel: s.stationLevels[0], lifetime: 0 },
    activePads: [],
    fillReady: false,
    onFillId: null,
    out: derived,
...
function brewSystem(c: TickCtx): void {
  const { dt, dishes, zonesOpen, stationLevels, tableLevels, readyCupsByZone, brewProgressByZone } = c;
...
function spawnSystem(c: TickCtx): void {
  const { npcs, tables, zonesOpen, tableLevels, dirty } = c;
  let nextId = c.nextId;
  let spawnTimer = c.spawnTimer;
  let spawnZone = c.spawnZone;
...
function serveSystem(c: TickCtx): void {
  const { s, zonesOpen, readyCupsByZone, carriedDirty, carriedDirtyFood, stats, liveNpcs, player } = c;
  let tray = c.tray;
...
function dishCycleSystem(c: TickCtx): void {
  const { zonesOpen, tray, trayFood, stats, trayCap, player } = c;
  let dishes = c.dishes;
  let cleanCups = c.cleanCups;
...
function waiterSystem(c: TickCtx): void {
  const { dt, s, derived, zonesOpen, obstacles, navGrid, readyCupsByZone, stats, dirty, liveNpcs, player } = c;
  let xp = c.xp;
...
function dishwasherSystem(c: TickCtx): void {
  const { dt, s, derived, zonesOpen, obstacles, navGrid, player } = c;
  let dishes = c.dishes;
  let cleanCups = c.cleanCups;
...
function revealSystem(c: TickCtx): void {
  const { dt, s, tables, zonesOpen, stationLevels, tableLevels, enqueueNotice, questIndex, requestFocus, input, player, padGate, activePads } = c;
  let notice = c.notice;

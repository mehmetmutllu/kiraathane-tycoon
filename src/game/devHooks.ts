...
export function installDevHooks(): void {
  ...
  window.__game = () => {
    ...
    return {
      ...
      // Tuvalet + depo (M4)
      hasWC: s.padsDone.includes('wc'),
      wcPaper: s.wcPaper,
      carriedPaper: s.carriedPaper,
      wcDoorPos: LAYOUT.wcDoor,
      depoPointPos: LAYOUT.depoPoint,
      cleanerPos: s.cleaner ? s.cleaner.pos.map((n) => +n.toFixed(2)) : null,
      cleanerTray: s.cleaner ? s.cleaner.tray : 0,

/**
 * glif-bak.mjs — `tools/glif-bak.html`i açar ve karesini alır (S19a).
 ...
 *
 * Kullanım: node tools/glif-bak.mjs [cikti.png]
 */
...
const cikti = process.argv[2] ?? 'docs/gorsel/ss/s19-glif.png';
...
const PORT = Number(process.env.BAK_PORT ?? (await bosPort(5320)));
const BEKLENEN_SURUM = Number(
  readFileSync(path.join(KOK, 'tools/glif-bak.html'), 'utf8').match(/const SURUM = (\d+)/)[1],
);
...
try {
  ...
  await p.goto(`http://127.0.0.1:${PORT}/tools/glif-bak.html`, { waitUntil: 'networkidle' });

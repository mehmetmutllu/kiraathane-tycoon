/**
 * olcum-nav-ab-t5b.mjs — T5b: NAV KARE KAZANCININ TARAYICI A/B'Sİ.
 *
 * ## Neden yeni araç
 * T5 node'da ×2,52 ölçtü (aynı süreç, donmuş oracle, 97.486 gerçek çağrı) ama KARE kazancı
 * tarayıcıda doğrulanamadı. Sebep ölçümün kendisiydi, kolun değil: T4 tabanı **gölge KAPALI**
 * alınmış, T5 koşusu gölge AÇIK, üstelik makine o gün genel olarak %22-38 yavaştı
 * (`docs/nav-raporu-t5.md` §6). **Farklı koşuları karşılaştırmak bu soruyu tanım gereği
 * cevaplayamaz.**
 *
 * Bu araç tek bir şey yapar: iki kolu **AYNI sayfada, AYNI dünyada, AYNI gölge durumunda,
 * dakikalar değil saniyeler arayla** koşturur.
 *
 * ## Tasarım — üçü bilerek
 * ① **ABBA deseni.** Düz ABAB, makine zamanla yavaşlıyorsa farkı olduğu gibi B'nin üstüne yazar.
 *    ABBA çifti lineer sürüklenmeyi **birinci mertebeden götürür**: A B B A dizisinde A'ların
 *    zaman merkezi ile B'lerin zaman merkezi çakışır.
 * ② **DENETİM KOLU.** Çizim ms · çizim çağrısı · üçgen · NPC — nav'la ilgisi olmayan kalemler.
 *    Bunlar iki kolda da aynı kalmalı; oynarsa ölçülen şey kol değil MAKİNEDİR ve koşu
 *    GEÇERSİZDİR. (§6'nın dersi: "makine genel olarak %22-38 yavaş" cümlesi ancak böyle bir
 *    denetimle yazılabilir — T5 koşusunda böyle bir kol yoktu, o yüzden sayı yorumlanamadı.)
 * ③ **ORAN, mutlak değil** (§G'nin dersi). Rapora `oracle/üretim` oranı girer; mutlak ms yalnız
 *    bağlam olarak yazılır ve BAŞKA BİR KOŞUYLA KARŞILAŞTIRILMAZ.
 *
 * ## İki sonuç da geçerlidir
 * Kol doğrulanırsa T5 kapanır. Çürütülürse kod yine doğru ve çıktı-eşdeğerdir (bekçi
 * `nav-kol-t5`: ~4.200 çift, 7/7 mutasyon) — yalnız "kareyi hızlandırdı" cümlesi geri alınır.
 * Araç hiçbir kolu kayırmaz: sıra ABBA'dır ve damga her dilimde kolun adını SAYFADAN okur.
 *
 * Koşu:  node tools/olcum-nav-ab-t5b.mjs             (kısa — geliştirirken)
 *        OLCUM=tam node tools/olcum-nav-ab-t5b.mjs   (tam — rapora yalnız bu girer)
 *        T5B_PORT=5219 ...      (port çakışırsa)
 *        T5B_GOLGE=kapali ...   (gölge kolunu sabitle; varsayılan acik)
 */
...
const CIKTI = path.join(KOK, 'docs/olcum-nav-ab-t5b' + EK + '.txt');
const CIKTI_JSON = path.join(KOK, 'docs/olcum-nav-ab-t5b' + EK + '.json');
const KORPUS = path.join(KOK, 'docs/olcum-nav-korpus-t5b' + EK + '.json');
...
async function main() {
  ...
  try {
    ...
    yaz('='.repeat(78));
    yaz('T5b NAV A/B (tarayici) · kip=' + rapor.kip + ' · ' + rapor.tarih);
    yaz('GPU: ' + surucu);
    ...
    yaz('Desen: ABBA x ' + BLOK + ' blok · dilim ' + DILIM_MS + ' ms · A=uretim B=oracle');
    yaz('='.repeat(78));
    yaz();

    // ISINMA: iki kol da bir kez koşar (JIT iki yolu da derlesin; ilk dilim avantajlı olmasın).
    await dilim(sayfa, 'uretim', 1200);
    await dilim(sayfa, 'oracle', 1200);

    const dizi = [];
    for (let b = 0; b < BLOK; b++) dizi.push('uretim', 'oracle', 'oracle', 'uretim');
    ...
    const B = olcumler.filter((d) => d.kol === 'oracle');
    ...
    yaz('  kalem            | uretim(A) | oracle(B) | fark %  | esik  | hal');
    /*
    ...
    yaz('  toplam cagri   : uretim ' + tr(toplam(A, 'navN')) + ' · oracle ' + tr(toplam(B, 'navN')));
    yaz('  nav ms/CAGRI   : uretim ' + f3(navCagri.a) + ' · oracle ' + f3(navCagri.b) + '  → oracle/uretim = x' + f2(oranCagri));
    yaz('  nav ms/KARE    : uretim ' + f2(navKare.a) + ' · oracle ' + f2(navKare.b) + '  → oracle/uretim = x' + f2(oranKare));
    yaz('  NODE TABANI (T5, ayni birim): x2,52 → tarayici '
      + (oranCagri >= 2.0 ? 'DOGRULUYOR' : oranCagri >= 1.3 ? 'KISMEN dogruluyor' : 'DOGRULAMIYOR'));
    yaz();

    yaz('§4 KARE — kolun kareye yansiyani');
    const kareKazanc = is.b > 0 ? ((is.b - is.a) / is.b) * 100 : 0;
    const navPayA = is.a > 0 ? (navKare.a / is.a) * 100 : 0;
    const navPayB = is.b > 0 ? (navKare.b / is.b) * 100 : 0;
    yaz('  karenin isi     : uretim ' + f1(is.a) + ' ms · oracle ' + f1(is.b) + ' ms  → uretim %' + f1(kareKazanc) + ' daha ucuz');
    yaz('  fps             : uretim ' + f1(fps.a) + ' · oracle ' + f1(fps.b));
    yaz("  nav'in KARE PAYI: uretim %" + f1(navPayA) + ' · oracle %' + f1(navPayB));
    ...
    yaz('  heap ayirma hizi : uretim ' + f1(ayirma.a) + ' MB/sn · oracle ' + f1(ayirma.b) + ' MB/sn'
      + '  → oracle/uretim = x' + f2(ayirma.a > 0 ? ayirma.b / ayirma.a : 0));
    yaz('  cagri basi       : uretim ' + f1(ayirmaCagri.a) + ' KB · oracle ' + f1(ayirmaCagri.b) + ' KB'
      + '   (oracle her cagrida ' + f1((izgara.hucre * 4) / 1024) + ' KB Int32Array ayirir)');

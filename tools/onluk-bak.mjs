/**
 * onluk-bak.mjs — `tools/onluk-bak.html`i açar ve karesini alır (S19a).
 ...
 *
 * Kullanım: node tools/onluk-bak.mjs [cikti.png]
 */
...
const cikti = process.argv[2] ?? 'docs/gorsel/ss/s19-onluk.png';
...
const BEKLENEN_SURUM = Number(
  readFileSync(path.join(KOK, 'tools/onluk-bak.html'), 'utf8').match(/const SURUM = (\d+)/)[1],
);
...
try {
  ...
  await p.goto(`http://127.0.0.1:${PORT}/tools/onluk-bak.html`, { waitUntil: 'networkidle' });

/**
 * patron-dokunus.mjs — `tools/patron-dokunus.html`i açar ve karesini alır (S19a).
 ...
 *
 * Kullanım: node tools/patron-dokunus.mjs [cikti.png]
 */
...
const cikti = process.argv[2] ?? 'docs/gorsel/ss/s19-patron.png';
...
const PORT = Number(process.env.BAK_PORT ?? (await bosPort(5280)));
const BEKLENEN_SURUM = Number(
  readFileSync(path.join(KOK, 'tools/patron-dokunus.html'), 'utf8').match(/const SURUM = (\d+)/)[1],
);
...
try {
  ...
  await p.goto(`http://127.0.0.1:${PORT}/tools/patron-dokunus.html`, { waitUntil: 'networkidle' });

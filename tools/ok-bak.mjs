/**
 * ok-bak.mjs — `tools/ok-bak.html`i açar ve karesini alır (S23).
 ...
 *
 * Kullanım: node tools/ok-bak.mjs [cikti.png]
 */
...
const cikti = process.argv[2] ?? 'docs/gorsel/ss/s23-ok-adaylari.png';
...
const PORT = Number(process.env.BAK_PORT ?? (await bosPort(5360)));
const BEKLENEN_SURUM = Number(
  readFileSync(path.join(KOK, 'tools/ok-bak.html'), 'utf8').match(/const SURUM = (\d+)/)[1],
);
...
try {
  ...
  await p.goto(`http://127.0.0.1:${PORT}/tools/ok-bak.html`, { waitUntil: 'domcontentloaded' });
  await p.waitForFunction(() => window.__OK_BAK_SURUM !== undefined, { timeout: 90000 });
  const surum = await p.evaluate(() => window.__OK_BAK_SURUM);
  if (surum !== BEKLENEN_SURUM) throw new Error(`BAYAT SAYFA: sunucu ${surum}, dosya ${BEKLENEN_SURUM} (port ${PORT})`);
  // Sekiz tuval kurulup ilk karelerini çizsin; işaretin "konuşan" katmanı damp ile açılıyor.
  await p.waitForTimeout(3000);
  await p.screenshot({ path: path.join(KOK, cikti), fullPage: true });

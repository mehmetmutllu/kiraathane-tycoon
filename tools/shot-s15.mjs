// shot-s15.mjs — S15 görsel doğrulaması: oyunu aç, mekânı büyüt, zamanı ilerlet ki müşteriler otursun,
// sonra hem geniş hem YAKIN kare al (oturuş çapası ve kafa ölçeği ancak yakında okunur).
...
try {
  ...
  await p.screenshot({ path: `${OUT}/s15-oyun-genis.png` });
  // OTURUŞ ÇAPASI yakın kırpma ile denetlenir: kalça taburenin oturağında mı, havada mı?
  await p.screenshot({ path: `${OUT}/s15-oturus-yakin.png`, clip: { x: 300, y: 180, width: 520, height: 420 } });
  ...
  if (yakin) {
    await p.waitForTimeout(1500);
    await p.screenshot({ path: `${OUT}/s15-oyun-yakin.png` });
  }

/**
 * yemek-bak.mjs — `tools/yemek-bak.html`i açar ve karesini alır (S19a).
 ...
 *
 * Kullanım: node tools/yemek-bak.mjs [cikti.png]
 */
...
const cikti = process.argv[2] ?? 'docs/gorsel/ss/s19-yemek.png';
...
const BEKLENEN_SURUM = Number(
  readFileSync(path.join(KOK, 'tools/yemek-bak.html'), 'utf8').match(/const SURUM = (\d+)/)[1],
);
...
try {
  ...
  const p = await b.newPage({ viewport: { width: 1360, height: 760 }, deviceScaleFactor: 2 });
  const hatalar = [];
  p.on('pageerror', (e) => hatalar.push(e.message.slice(0, 160)));
  p.on('console', (m) => {
    if (m.type() === 'error') hatalar.push(m.text().slice(0, 160));
  });
  await p.goto(`http://127.0.0.1:${PORT}/tools/yemek-bak.html`, { waitUntil: 'networkidle' });

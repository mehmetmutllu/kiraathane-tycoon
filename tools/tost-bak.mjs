/**
 * tost-bak.mjs — T8b G-90: tost noktası adaylarını AYNI KADRAJDA çizer (`tools/tost-bak.html`).
 * Sunucuyu kendi kaldırır (model-bak.mjs'in kopyası). Kullanım: node tools/tost-bak.mjs [çıktı.png]
 */
...
const [cikti = 'docs/gorsel/ss/t8b-tost-aday.png'] = process.argv.slice(2);
...
const url = `http://127.0.0.1:${PORT}/tools/tost-bak.html`;
const bekle = async () => {
  for (let i = 0; i < 60; i++) {
    try {
      const r = await fetch(`http://127.0.0.1:${PORT}/tools/tost-bak.html`);
      if (r.ok) return true;
    } catch {
      /* henüz ayakta değil */
    }
...
const sayfa = await tarayici.newPage({ viewport: { width: 1400, height: 820 }, deviceScaleFactor: 2 });
const hatalar = [];

...
| `kaykit-holiday-bits/` | 5 model (`chair_large_red/green/blue/brown` · `carpet_round_small`) + `holiday_bits_texture.png` | KayKit Holiday Bits 1.0 — Kay Lousberg | **CC0** — pakette `License.txt` YOK, lisans itch sayfasının kendi alanından: *Creative Commons Zero v1.0 Universal* | ✅ **F4c-2'de GERİ GELDİ (2026-09-24, D-155)** — 💎 yılbaşı koltuğu (4 renk) + yuvarlak halı; yalnız kullanılan 5 model (0,2 MB). F2'de (D-125) ölü yük diye çıkarılmıştı. |
...
| ~~`kaykit-prototype-bits/`~~ **SÖKÜLDÜ (T7, 2026-09-23)** | 3 model (paketin 72'sinden: `Door_A` · `Door_A_Decorated` · `Door_B`) + `prototypebits_texture.png` | KayKit Prototype Bits 1.1 — Kay Lousberg | **CC0** (pakette `License.txt`) | ✅ eklendi 2026-09-14 (S13) · WC kabin kapısıydı; **T7 (2026-09-23) kabin elle çizime geçti, bugün kullanılmıyor** |
